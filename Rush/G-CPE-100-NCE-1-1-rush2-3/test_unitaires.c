...
Test(step, test_verif_arguments)
{
    char *argv[] = {"./a.out", "The Hello World"};
    int return_test;

    return_test = step(2, argv);
    cr_assert_eq(return_test, 84);
}

Test(step, test_verif_script_without_error)
{
    char *argv[] = {"./a.out", "The Hello World", "a"};
    int return_test;

    return_test = step(3, argv);
    cr_assert_eq(return_test, 0);
}

Test(step, test_verif_script_without_error2)
{
    char *argv[] = {"./a.out", "The Hello World", "a", "b", "c", "d", "e", "f", "g"};
    int return_test;

    return_test = step(9, argv);
    cr_assert_eq(return_test, 0);
}
